 ...
 ifneq ($(filter $(dtb_capsule_pkg_name),$(packages_enabled)),)
	...
	if [ $* = $(firstword $(flavours)) ] ; then \
		...
		for machine in hamoa purwa ; do \
			case $$machine in \
				hamoa) fmp_guid=0F6D58FC-2258-4D27-9E23-D77219B0897C ;; \
				purwa) fmp_guid=185a798b-13b2-4595-bd08-e2770a4bb190 ;; \
			esac ; \
			...
			( cd $$mdir && \
			  $$QCT create \
				...
				-config config.json \
				-p $(CURDIR)/$(DEBIAN)/certs/QcFMPCert.pem \
				-x $(CURDIR)/$(DEBIAN)/certs/QcFMPRoot.pub.pem \
				-oc $(CURDIR)/$(DEBIAN)/certs/QcFMPSub.pub.pem \
				-guid $$fmp_guid \
